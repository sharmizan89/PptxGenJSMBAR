...
/**
 * Generates XML for only the image objects in a layout's _slideObjects array.
 * Used to inject images (like logos) into pre-built layout XML.
 * @param {SlideLayout} layout - layout object with _slideObjects
 * @return {string} XML fragment containing only the image shapes
 */
export function generateLayoutObjectsXml(layout: SlideLayout): string {
	let strXml = ''
	
	layout._slideObjects.forEach((slideItemObj: ISlideObject, idx: number) => {
		// Only handle images for now (logos are added as images)
		if (slideItemObj._type === SLIDE_OBJECT_TYPES.image) {
			const x = slideItemObj.options?.x ? getSmartParseNumber(slideItemObj.options.x, 'X', layout._presLayout) : 0
			const y = slideItemObj.options?.y ? getSmartParseNumber(slideItemObj.options.y, 'Y', layout._presLayout) : 0
			const cx = slideItemObj.options?.w ? getSmartParseNumber(slideItemObj.options.w, 'X', layout._presLayout) : 914400
			const cy = slideItemObj.options?.h ? getSmartParseNumber(slideItemObj.options.h, 'Y', layout._presLayout) : 914400
			
			strXml += '<p:pic>'
			strXml += '  <p:nvPicPr>'
			strXml += `<p:cNvPr id="${100 + idx}" name="${slideItemObj.options?.objectName || 'Image ' + idx}" descr="${encodeXmlEntities(slideItemObj.options?.altText || slideItemObj.image || '')}"/>`
...
function genXmlParagraphProperties (textObj: ISlideObject | TextProps, isDefault: boolean): string {
	...
	// A: Build paragraphProperties
	{
		...
		if (typeof textObj.options.bullet === 'object') {
			...
			// PATCH: For placeholders, don't add indent/marL/buNone - let layout styles be inherited
			const isPlaceholder = textObj.options.placeholder || (textObj.options as ObjectOptions)._placeholderType || (textObj.options as ObjectOptions)._placeholderIdx
			if (!isPlaceholder) {
...
function genXmlBodyProperties (slideObject: ISlideObject | TableCell): string {
	...
	const opts = slideObject.options as ObjectOptions

	// PATCH: For placeholders, use empty bodyPr to inherit ALL formatting from layout
	// Only use explicit bodyPr if user provided explicit margin/inset/fit options
	const isPlaceholder = slideObject._type === SLIDE_OBJECT_TYPES.placeholder
	if (isPlaceholder) {
		// Check if user explicitly set any text box formatting options
		const hasUserProvidedFormatting = opts.margin !== undefined || opts.inset !== undefined || opts.fit !== undefined || opts.valign !== undefined
		if (!hasUserProvidedFormatting) {
			return '<a:bodyPr/>'
		}
	}
...
export function genXmlTextBody (slideObj: ISlideObject | TableCell): string {
	...
	// STEP 2: Add bodyProperties
	{
		...
		// PATCH: For placeholders, use empty lstStyle to inherit formatting from layout
		else if (slideObj._type === 'placeholder') strSlideXml += '<a:lstStyle/>'
